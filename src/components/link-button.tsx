...
export const LinkButton = ({ small, newTab, className, children, icon, ...props }: Props) => {
  ...
  return (
    <Link
      className={cn(
        'rounded-full w-fit p-0.5 relative flex items-center justify-center overflow-clip group hover:scale-102 active:scale-90 active:brightness-50 transition',
        className
      )}
      {...target}
      {...props}
      ...
      <div
        className={cn(
          ...
          small
            ...
            : '[&>svg]:size-8 [&>svg]:max-w-8 text-xl px-7 py-2'
        )}
      >
        {icon && <Icon name={icon} />}
        {children}
